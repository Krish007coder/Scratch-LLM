...
This installs `flask` and `requests` — both lightweight, standard packages.
...
```
VectorDB/
├── main.py             ← Entry point (load demo data, start Flask server)
├── server.py           ← Flask HTTP server (all REST routes)
├── vectordb/
│   ├── __init__.py     ← Package exports
│   ├── types.py        ← VectorItem, DocItem dataclasses
│   ├── metrics.py      ← euclidean, cosine, manhattan distance functions
│   ├── bruteforce.py   ← BruteForce KNN — O(N·d), exact baseline
│   ├── kdtree.py       ← KD-Tree — O(log N), axis-aligned partitioning
│   ├── hnsw.py         ← HNSW — O(log N), multilayer small-world graph
│   ├── vectordb.py     ← VectorDB — unified interface over all 3 (16D)
│   ├── documentdb.py   ← DocumentDB — HNSW over real Ollama embeddings (768D)
│   └── ollama_client.py ← HTTP client → /api/embeddings + /api/generate
...
| `python: command not found` | Add Python to your system PATH |
...
Then edit [ollama_client.py](vectordb/ollama_client.py):
```python
gen_model: str = "llama3.2:1b"   # change this
```
